      ...
      <div class="col-md-12 mt-4">
        ...
        <div class="container">
          <table class="table table-striped table-bordered table-hover mt-6">
            ...
            <tbody>
            @forelse ($stokproduks as $stokproduk)
            <tr>
                <th scope="row" class="text-center">{{$loop->iteration}}</th>
                <td class="text-center">{{$stokproduk->nama_produk}}</td>
                <td class="text-center">{{$stokproduk->harga}}</td>
                <td class="text-center">{{$stokproduk->stok}}</td>
                <td class="text-left">{{$stokproduk->keterangan}}</td>
                <td class="text-center">
                    @if($stokproduk->gambar)
                    <img src="{{ asset('storage/uploads/' . $stokproduk->gambar) }}" style="max-width: 100px; max-height: 100px;" alt="Gambar Produk">
                    @else
                    <span>Tidak ada gambar</span>
                    @endif
                <td>
                    <a href="{{route('edit.stok', ['id' => $stokproduk->id]) }}" class="btn btn-sm btn-primary mb-2"><i class="fas fa-edit"></i>Update</a>
                    <form action="{{ route ('delete.stok', ['id' => $stokproduk->id])}}" method="POST">
                    @method('DELETE')
                    @csrf
